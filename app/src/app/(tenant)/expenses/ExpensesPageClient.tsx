...
export default function ExpensesPageClient({ tenantId, bankAccounts, recentEntries }: Props) {
  const [showForm, setShowForm] = useState(false)
  const [entries, setEntries] = useState(recentEntries)
  const [saved, setSaved] = useState(false)

  async function handleSubmit(data: Parameters<React.ComponentProps<typeof ExpenseEntryForm>['onSubmit']>[0]) {
    ...
    <div className="max-w-5xl mx-auto py-8 px-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Record money spent on business expenses</p>
        </div>
        <button
      ...
      {saved && (
        <div className="mb-4 bg-green-50 border border-green-200 text-green-800 text-sm px-4 py-3 rounded-lg">
          Expense saved. Accounting entries created automatically.
        </div>
      )}

      {showForm && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 mb-8">
          <h2 className="text-lg font-semibold text-slate-800 mb-6">Enter Expense Details</h2>
          <ExpenseEntryForm bankAccounts={bankAccounts} onSubmit={handleSubmit} />
        </div>
      )}
                ...
                <tr key={e.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium
                      ${e.status === 'posted' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                      {e.status}
                    </span>
                  </td>
                </tr>
