...
export default function SalesPageClient({ tenantId, customers, bankAccounts, recentEntries }: Props) {
  const [showForm, setShowForm] = useState(false)
  const [entries, setEntries] = useState(recentEntries)
  const [saved, setSaved] = useState(false)

  async function handleSubmit(data: Parameters<React.ComponentProps<typeof SaleEntryForm>['onSubmit']>[0]) {
    ...
    <div className="max-w-5xl mx-auto py-8 px-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Record money earned from selling goods or services</p>
        </div>
        <button
      ...
      {saved && (
        <div className="mb-4 bg-green-50 border border-green-200 text-green-800 text-sm px-4 py-3 rounded-lg">
          Sale entry saved successfully. Accounting entries created automatically.
        </div>
      )}
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 mb-8">
          <h2 className="text-lg font-semibold text-slate-800 mb-6">Enter Sale Details</h2>
          <SaleEntryForm
            customers={customers}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {entries.length === 0 ? (
          <div className="px-6 py-12 text-center text-slate-400">
            ...
            <p className="text-sm mt-1">Click &quot;New Sale&quot; to record your first sale entry.</p>
          </div>
        ) : (
